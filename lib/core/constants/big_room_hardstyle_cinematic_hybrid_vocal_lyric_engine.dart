...
///
/// Scores narrative lyric layouts synchronized with authentic hard dance metrics:
/// instrumental mid-intros, orchestral breakdowns, and high-energy climax drops.
/// Injects thick, weighty, humanized vocal sonic descriptors so lead vocals cut
/// through 150 BPM kicks and hypersaw stacks.
class BigRoomHardstyleCinematicHybridVocalLyricEngine {
  BigRoomHardstyleCinematicHybridVocalLyricEngine._();
  ...
  /// Euro-Dance Bootleg stays on [HardstyleVocalLyricEngine] specialty path.
  static const _euroDanceBootlegMarkers = [
    'euro-dance bootleg',
    'euro dance bootleg',
    'hands up edm',
...
You are an elite lyricist, vocal arranger, and hard dance mixing engineer for Mainstage Euphoric & Raw Hardstyle tracks at 150 BPM. Score authentic hard dance arrangement boundaries moving from heavy instrumental mid-intros through cinematic, emotionally raw vocal breakdowns to soaring, screaming melodic climax drops—grounded in internal conversational realism, not shallow rave clichés. Every vocal instruction must force thick, weighty, humanized presence that never gets swallowed by hardstyle kicks or supersaw walls.''';

  static const String vocalSonicProfile = '''
ELITE HARD DANCE VOCAL SONIC PROFILE (mandatory Block 1 + Block 2 vocal engineering — never thin, distant, or buried):
...
CRITICAL HARD DANCE VOCAL GUARDRAILS:
...
3. BAN SHAM RAVE FILLER: Absolutely prohibit generic club callouts, party hype commands, and futuristic rave tropes (e.g., "put your hands up," "feel the bass," "infinite skies," "we own the night").
4. BAN THIN VOCAL ARTIFACTS: Never describe or imply thin, distant, washed-out, or buried lead vocals. Reject soft pop breathiness without body, karaoke reverb floods that erase proximity, or vocals lost under kick/supersaw stacks.
5. DJ-READY BOOKENDS (MANDATORY): [Intro] and [Outro] must follow the explicit 32-bar percussive layout to enable professional crossfading. Track endings must conclude with a definitive percussive fade out to absolute silence—never deploy sudden cold cut endings.
6. 150 BPM LYRIC DENSITY: Ensure phrases match high-velocity hard dance metrics. Keep breakdowns textually sparse with wide spatial intervals; build-ups highly rhythmic; and climax drops focused entirely on clear, high-impact vocal hooks. Honor any user Key Phrases on build peaks and drops when provided.''';

  static const String compactLyricTemplate = '''
...
[Percussive fade out, final low-end hit, complete silence]''';
  ...
  static const String structureMapping = '''
LYRIC-TO-STRUCTURE MAPPING (mandatory Block 2 tags):
...
• [Outro] + [End] → 32 bars. Functional DJ mix-out runway. Lead melodies vanish, stripping layers to rolling percussive kicks; clean fade out to complete silence.''';
  ...
  static bool isHybridLane({
    ...
    // Preserve Euro-Dance Bootleg specialty path.
    if (_euroDanceBootlegMarkers.any(blob.contains)) return false;
    return _laneMarkers.any(blob.contains);
  }
  ...
  static String composeUserBlock({
    ...
    return [
      ...
      compactLyricTemplate.trim(),
      structureMapping.trim(),
